...
- **15 live/preview route comparisons pass**, including an explicit rendered Shopify theme ID check. Amazon destinations and full attribution parameters, Klaviyo embed IDs, external loaders, integration markers and protected native code are preserved. Shopify adds its preview hot-reload script on the connected theme; no existing loader is removed and no new business integration loader appears.

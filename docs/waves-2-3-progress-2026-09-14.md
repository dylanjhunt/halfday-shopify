...
The follow-up pass verified four published desktop product pages, Peach review pagination/form opening, and Strawberry's no-review fallback. The empty Yotpo price metadata is confirmed on all four samples; its fix remains unpublished. CLI confirmed the preview capacity is still full. The earlier Klaviyo access block was resolved when Dylan selected Halfday. A fresh read-only account audit and isolated footer routing draft are complete. Retain the existing welcome design; the replacement creative is inactive. See the [account evidence and staging record](wave-2/account-audit-and-staging-2026-09-14.md).

September 14, 2026. Branch **`djh/wave-3-review-prep`** includes the existing Wave 2 email package, current main through `9ac3fc5` and the work below. Main's three newer Shopify/Locksmith commits were incorporated into the development branch. **No main push, production theme upload, live app setting change, order, signup or email send was performed.**
...
| --- | --- | --- |
| Wave 2 creative scope | Existing welcome design retained. Earlier local redesign is inactive; one unused Draft clone and one unused standalone template are recorded. | [Account staging record](wave-2/account-audit-and-staging-2026-09-14.md) |
| Signup and offer | Footer routing correction persisted in isolated Draft S4DvQh; original remains Live to HelloFresh. Offer and unsubscribe intent await review. | [Signup specification](wave-2/signup-and-offer.md), [staging manifest](wave-2/staging-manifest.json) |
...
- **Klaviyo is now verified in Halfday.** Live footer routing still points to HelloFresh; the correction is staged separately. Newsletter unsubscribe scope differs from the account default. Real Viewed Product events include public Peach in `Employee Shop`, so that category cannot safely exclude operational traffic. Faire order tags still need mapping to actual profile fields.

## What cannot be tested or applied solely in a dev theme

| Remaining work | Required next input | Prepared next action |
| --- | --- | --- |
| Klaviyo eligibility and minimal change packet | Confirm unsubscribe intent and operational identity ownership | Finish native type/eligibility and queue/bridge checks; retain existing creative and leave every live setting untouched |
...
- Earlier local email layouts were audited but are no longer the proposed welcome design. Two real browse-event detail panels were inspected; native dynamic binding and inbox delivery are not certified. One standalone template was imported and previewed without sending before redesign work stopped.
...
1. Confirm product/operations ownership, offer terms and unsubscribe intent. Existing welcome design stays.
2. Review the isolated footer routing draft; complete eligibility/queue/bridge checks and prepare an authorized signup test.

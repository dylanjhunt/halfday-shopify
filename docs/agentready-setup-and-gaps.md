...
**Current status, September 9 after the deployed fix:** Apply now has a durable 34/34 receipt. Current development homepage Agent JSON uses the reviewed retailer description and accurate no-return/retailer-shipping summaries. The development embed and Agent JSON are enabled for brand-only output; production embed and JSON-LD stay off, with no catalog approvals. Curated llms.txt was regenerated and verified. Contact-field UI persistence remains inconsistent even though support email is present in generated output. Approved product output, external-retailer offers, JSON-LD ownership and MCP/ACP still require verification. [Latest findings](agentready-product-feedback.md). Earlier dated reproductions below are historical.

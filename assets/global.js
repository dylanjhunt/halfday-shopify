...
class collectionGridItem extends HTMLElement {
  constructor() {
      ...
      this.mainCollection = document.querySelector("[js-hb-main-collection]");
      this.desktopPosition = parseInt(this.dataset.hbPositionDesk);
      this.mobilePosition = parseInt(this.dataset.hbPositionMob);
  ...
  appendGridItem() {
      const targetProductItem = this.mainCollection.querySelector(`[js-product-grid] .grid__item:nth-child(${document.documentElement.clientWidth > 767 ? this.desktopPosition : this.mobilePosition})`);
      
      if(targetProductItem) {
          targetProductItem.insertAdjacentHTML("afterend",this.innerHTML)
      }
  };
}

...
- Published baseline: `baseline/live-wave-1-2026-09-09` (`bb0169f`), audited and deployed with Dylan’s authorization. The first Wave 1 release is retained at `baseline/live-wave-1-2026-09-08` (`2c8ff4f`). The original pre-work theme remains at `baseline/live-2026-09-07` and Shopify theme `141825474760` as a rollback reference.
- Continue Shopify CLI work on descriptive `djh/...` branches (older work uses `feature/...` or `fix/...`) and development/unpublished themes. Current Wave 2/3 review branch: `djh/wave-3-review-prep`, incorporating the Wave 2 email package and main through `9ac3fc5`. Pending Wave 1 PR #2 still owns development theme `142755430600`; do not overwrite it. September 14 new unpublished-theme creation failed at the 20-theme limit, so the Wave 3 review-price fix is local only. See `docs/waves-2-3-progress-2026-09-14.md`. Re-resolve theme roles each session. Preserve audit history in `docs/wave-1-progress.md`. Otherwise preview and review before an authorized release.
...
- Audit priorities: availability messaging, 4-packs/slim cans, Klaviyo, SEO/AEO, legacy content, Cin7/ShipStation/AfterShip/Faire, and maintenance of the retained Yotpo review integration. Dylan confirmed Yotpo should stay; do not add Caffine Functions or resume a replacement migration. The original Walmart/Target syndication request is deferred separately, not automatically satisfied or cancelled. Do not activate flows, uninstall apps, or alter integrations as part of an audit.
...
- September 14 Klaviyo scope: retain the existing welcome design; do not rebuild it. No live flow, message, form, consent, audience or sending changes. Use plans or isolated non-sending Draft objects only. Current account evidence and the three staging/unused objects are recorded in `docs/wave-2/account-audit-and-staging-2026-09-14.md` and `docs/wave-2/staging-manifest.json`. Neither a dev theme nor Manual flow status is a sending sandbox.

...
- Latest main baseline: `2893a6f`, Dylan’s merge of PR #1 on September 9. The preceding `baseline/live-wave-1-2026-09-09` (`bb0169f`) remains a release reference. The first Wave 1 release is retained at `baseline/live-wave-1-2026-09-08` (`2c8ff4f`). The original pre-work theme remains at `baseline/live-2026-09-07` and Shopify theme `141825474760` as a rollback reference.
- Continue Shopify CLI work on descriptive `djh/...` branches (older work uses `feature/...` or `fix/...`) and development/unpublished themes. Current follow-through branch: `djh/wave-1-contrast-agentready`; development theme: `142755430600` (re-resolve its role each session). Preserve audit history and track work in `docs/wave-1-progress.md`. Production repair is allowed only for confirmed broken functionality; otherwise preview and review before an authorized release.
...
## Current Agentready development configuration

The development embed is enabled for verified brand-only Agent JSON. Shared Agent JSON is on; JSON-LD, catalog channel approvals and live embed remain off. No product/collection handles are approved. Keep those exclusions until retailer-safe catalog output is separately verified. Run the regression script with `--agentready-embed enabled` for this development configuration; the default still expects disabled. Do not enable live by changing shared app settings or pushing main without release authorization.
